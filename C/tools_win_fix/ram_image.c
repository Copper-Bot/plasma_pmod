...
int main(int argc, char *argv[])
{
   ...
   for(count = 0; count < RAM_ROWS_TOTAL*RAM_DWORDPERROW; ++count)
   {
	...
	// BLG PATCH
	if( ! feof(file) ){
		printf("- THE PROGRAM IS TOO LONG FOR THE RAM MEMORY !\n");
		return -1;
   }
	// END OF BLG PATCH
   fclose(file);

	// PRINTING INFORMATION + CHANGING MEMORY SIZE
	printf("- BINARY PROGRAM SIZE : %d INSTRUCTIONS\n", count);
	printf("- MEMORY REQUIREMENT  : %d KBYTES\n", (count*4));
	int full = (count*4)/8192;
	int part = (4*count) - (full * 8192);
	int nbBanks = full + (part > 0) /* + 2*/;
	nbBanks = (nbBanks>16)?16:nbBanks;
	printf("- # OF MEMORY BANKS   : %d\n", nbBanks);

	char  chai[] =  "block_count : integer := ";
	char* mPtr = strstr(buf, chai);
   if(mPtr == NULL)
   {
		printf(" Input file error, /block_count/ not found\n");
      return -1;
   }
	mPtr[ sizeof("block_count : integer :=") + 0 ] = ('0' + nbBanks / 10);
	mPtr[ sizeof("block_count : integer :=") + 1 ] = ('0' + nbBanks % 10);
	// END OF PRINTING INFORMATION + CHANGING MEMORY SIZE
   ...
   for(i = 0; i < count; ++i)
   {
      sprintf(text, "%8.8x", code[i]);
		//printf("Writing /%8.8x/ at %u\n", code[i], &ptr_list[index][iposinrow]);
      index = iblock*RAM_ROWS*RAM_SPLIT+irowinsplit; 
		//printf(" - Writing /%c/ at %u\n", text[0], &ptr_list[index][iposinrow]);
		//printf(" - Writing /%c/ at %u\n", text[1], &ptr_list[index][iposinrow+1]);
		//printf(" - Writing /%c/ at %u\n", text[2], &ptr_list[index+RAM_ROWS][iposinrow]);
		//printf(" - Writing /%c/ at %u\n", text[3], &ptr_list[index+RAM_ROWS][iposinrow+1]);
		//printf(" - Writing /%c/ at %u\n", text[4], &ptr_list[index+RAM_ROWS*2][iposinrow]);
		//printf(" - Writing /%c/ at %u\n", text[5], &ptr_list[index+RAM_ROWS*2][iposinrow+1]);
		//printf(" - Writing /%c/ at %u\n", text[6], &ptr_list[index+RAM_ROWS*3][iposinrow]);
		//printf(" - Writing /%c/ at %u\n", text[7], &ptr_list[index+RAM_ROWS*3][iposinrow+1]);
