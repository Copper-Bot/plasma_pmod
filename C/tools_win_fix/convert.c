...
int main(int argc, char *argv[]) {
	...
	printf("================bin/text converter====================\n");
	printf("INPUT\t\t-> %s\n", input);
	printf("OUTPUT_BIN\t-> %s\n", output_bin);
	printf("OUTPUT_TXT\t-> %s\n", output_txt);
	printf("======================================================\n");
	//printf("%s -> %s/%s\n", input, output_bin, output_txt);
	infile = fopen(input, "rb"); // rb = Read Byte mode
	if (infile == NULL) {
...
//		printf("Ligne code avant %d : 0x%02x\n", test, code[test]);
//	}
	...
	for (i = 0; i < elfHeader->e_phnum; ++i) {
		...
		if (elfProgram->p_vaddr < BUF_SIZE) {
			...
			printf("[P vaddr=0x%x,P offset=0x%x,P filesz=0x%x,P memsz=0x%x,P flags=0x%x]\n",
				   elfProgram->p_vaddr, elfProgram->p_offset,
				   elfProgram->p_filesz, elfProgram->p_memsz,
				   elfProgram->p_flags);
			printf("[P code=0x%p,P P buf=0x%p]\n",
				   &code, &buf);
			if ((int) elfProgram->p_vaddr < 0)
				elfProgram->p_vaddr = 0;


			for (int test = 0; test < 200; test += 4) {
				printf("Ligne avant export %d :\t", test);
				printf(" code 0x%02x%02x%02x%02x", code[test], code[test + 1], code[test + 2], code[test + 3]);
				printf(" buf 0x%02x%02x%02x%02x\n", buf[test], buf[test + 1], buf[test + 2], buf[test + 3]);
			}
...
//				code[ind] = buf[ind + elfProgram->p_offset];
//			}



			for (int test = 0; test < 200; test += 4) {
				printf("Ligne après export %d :\t", test);
				printf(" code 0x%02x%02x%02x%02x", code[test], code[test + 1], code[test + 2], code[test + 3]);
				printf(" buf 0x%02x%02x%02x%02x\n", buf[test], buf[test + 1], buf[test + 2], buf[test + 3]);
			}
...
//	for (int test = 0; test < 30; test++) {
//		printf("Ligne code après %d : 0x%02x\n", test, code[test]);
//	}
...
//	for (int test = 0; test < 100; test+=4) {
//		printf("Ligne code avant export %d : 0x%02x%02x%02x%02x\n", test, code[test],code[test+1],code[test+2],code[test+3]);
//	}
...
//   fclose(txtfile);
	...
	printf("complete length = %d octets = 0x%x => %d RAMs\n", bss_end, bss_end,
		   (bss_end / 8192 + ((bss_end % 8192) != 0 ? 1 : 0)));
